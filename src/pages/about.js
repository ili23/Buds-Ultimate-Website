...
const createMobileSection = (header, text, image, alt, linebreak, shadow) => {
  return (
    <div className="py-8">
      <h1 className="text-2xl pb-6">{header}</h1>
      <img className={shadow} alt={alt} src={image}></img>
      <div className="text-sm pt-8">{text}</div>
      {linebreak !== "" && <div className="text-sm pt-5">{linebreak}</div>}
    </div>
  );
};
const createRightImageSection = (
  header,
  text,
  image,
  alt,
  ...
  return (
    <div className="grid grid-cols-12 py-28">
      <div className="col-start-1 col-span-5 ">
        ...
        {linebreak !== "" && <div className="text-xl pt-5">{linebreak}</div>}
      </div>
...
const About = () => {
  return (
    <main className="bg-[#1E1E1E] h-max w-screen">
      <NavbarAndSocial />
      <div className="md:hidden px-[10%] pb-[5%]">
        {createMobileSection(
          Data[0][0],
          Data[0][1],
          snow,
      ...
      <div className="hidden md:block px-[10%] pb-[5%]">
