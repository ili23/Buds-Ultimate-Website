...
const Archive = () => {
	const yearCard = (year, img, coaches, captains, players) => {
		...
		const secondHalfPlayerList = secondHalfPlayer.map((name) => <li>{name}</li>);
		return (
			<div className="grid grid-cols-12 mb-28">
				<div className="col-start-1 col-end-3">
					<h1 className="text-4xl pb-8">{year}</h1>
					<h2 className="text-lg underline ">Captains:</h2>
					<ul className="text-sm pb-6">{captainList}</ul>
					{coaches.length !== 0 && <h2 className="text-lg underline">Coaches:</h2>}
					<ul className="text-sm pb-6">{coachList}</ul>
				</div>
				<div className="col-start-4 col-end-6">
					<ul className="text-base">{firstHalfPlayerList}</ul>
				</div>
				<div className="col-start-6 col-end-8">
					<ul className="text-base">{secondHalfPlayerList}</ul>
				</div>
	...
	const rosterView = Roster.map((year) => <div>{yearCard(year[0], year[1], year[2], year[3], year[4])}</div>);
	return (
		<main className="bg-[#1E1E1E] h-fit w-screen">
			<NavbarAndSocial />
			<div className="px-[10%] my-10">
				<h1 className="text-5xl pb-20">Past Buds</h1>
				<div>{rosterView}</div>
			</div>
			<Footer />
		</main>
